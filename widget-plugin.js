function preFormat($) {
	let _ = window.decodeURI($);
	_ = _.split("#")[0], "/" !== $.slice(-1) && (_ += "/");
	let t = _.split("?");
	return 2 == t.length && (t[1] = t[1].split("&").join("/"), _ = "/" !== t[0].slice(-1) ? t[0] + "/" + t[1] : t[0] + t[1]), _
}

function generateExp($) {
	let _ = preFormat($),
		t = "";
	return t = "^", t += _.replace(/\{\{([a-z_]+)\}\}/g, "AISENSY").replace(/[-[\]{}()*+?.,\\^$|#\s/\\]/g, "\\$&").replace(/AISENSY/g, "([^\\/]+)"), t += "$"
}
...
function CreateWhatsAppButtonAndWidget($) {
	...
	if (null != a) {
		...
		let m = {};
		m["{{page_url}}"] = `<a href=${n} target="_blank">${n}</a>`, m["{{page_title}}"] = document.title, t = p.userMsg;
		let d = extractParams(p.url),
			h = extractParamsValue(preFormat(n), p.regEx);
		d.length == h.length && d.forEach(function($, _) {
			let t = h[_];
			p.toFormat && (t = t.replace(/[\-_]/g, " ")), p.toCapitalize && t && (t = t.split(" ").map(function($) {
				return $[0].toUpperCase() + $.slice(1)
			}).join(" ")), m[d[_]] = t
		}), Object.keys(m).forEach(function($) {
			let _ = $.replace(/[-[\]{}()*+?.,\\^$|#\s]/g, "\\$&");
			t = t.replace(RegExp(`${_}`, "g"), m[$])
		})
	}
	let g = `
	...
	`,
		...
		.df-btn {
			...
			bottom: ${_.mb}px !important;
			${"Bottom-Right"==_.position?"right:"+_.mr:"left:"+_.ml}px !important;
			display: flex !important;
			flex-direction: column !important;
			z-index: 10005 !important;
			align-items: ${"Bottom-Right"==_.position?"flex-end":"flex-start"} !important;
		...
		.df-btn-text {
			...
			padding-right: ${_.btnCTA?"15px":"5px"} !important;
			padding-left: ${_.btnCTA?"10px":"5px"} !important;
			padding-top: 5px !important;
			padding-bottom: 5px !important;
			font-size: 16px !important;
			margin-top: 15px !important;
			background-color: ${_.btnBackground};
	...
	`,
		b = document.getElementById("df-style");
	b || ((b = document.createElement("style")).id = "df-style"), document.getElementById("df-style") || document.body.appendChild(b), b.innerHTML = x;
	let f = document.getElementById("df-btn-cont");
	f || ((f = document.createElement("div")).id = "df-btn-cont"), f.innerHTML = `
		<div class="df-btn df-closed" >
			<div class="df-btn-content">
				<div class="df-content-topbar">
					<div class="df-brand-img">
						<img src="${_.brandImgUrl}" height="100%" width="100%" style="border-radius: 50%;" alt="img"/>
					</div>
					<div style="flex-grow: 1;">
						<div class="df-brand-name">${_.brandName}</div>
						<div class="df-brand-sub">${_.brandSub}</div>
					</div>
				...
				<div class="df-content-window">
					<div class="df-window-msg-cont">
						...
						${_.onscreenImg?'<div class="df-onscreen-img"></div>':""}
						<pre class="df-window-msg">${t||_.onscreenMsg}</pre>
					</div>
				</div>
				<div class="df-content-bottombar">
					...
					<div class="df-bottombar-branding">
						...
						/>
						by 
						<a href="javascript:void(0)"> <i>AestheticBits</i> </a>
					</div>
				</div>
			</div>
	...
	`, document.getElementById("df-btn-cont") || document.body.appendChild(f), console.log("before", {
		dfToggled: window.dfToggled
	}), "dev" != _.env ? window.dfToggled = !1 : window.dfToggled = !window.dfToggled, console.log("after", {
		dfToggled: window.dfToggled
	});
	let w = window.innerWidth,
		v = w < 700 && "false" == _.widgetOnMobile;
	if (window.dfToggle = ($ = !1) => {
			if ("WhatsAppBtn" != _.variant || (document.querySelector(".df-btn-text").innerHTML = s + (_.btnCTA ? " " + _.btnCTA : ""), _.btnCTA ? document.querySelector(".df-btn-text").classList = "df-btn-text" : document.querySelector(".df-btn-text").classList = "df-btn-text df-btn-text-icon-only", $ || goToLink()), "WhatsAppBtnAndChatWidget" == _.variant) {
				if (v) $ || goToLink();
				else {
					document.querySelector(".df-btn-text").innerHTML = dfToggled ? s + (_.btnCTA ? " " + _.btnCTA : "") : s, document.querySelector(".df-btn").classList = dfToggled ? "df-btn df-closed" : "df-btn", dfToggled && _.btnCTA ? document.querySelector(".df-btn-text").classList = "df-btn-text" : document.querySelector(".df-btn-text").classList = "df-btn-text df-btn-text-icon-only";
					let t = checkWidgetSession();
					dfToggled && !t && createWidgetSession()
				}
			}
			dfToggled = !dfToggled
		}, window.goToLink = () => {
			if ("dev" != _.env && "create.aisensy.com" != window.location.hostname) {
				let $ = `https://wa.aisensy.com/${_.linkid}?url=${window.location.href}&title=${document.title}`;
				window.open($)
			}
		}, "dev" == _.env && dfToggle(!0), window.openWidgetOnLoad = () => {
			setTimeout(function() {
				dfToggled || dfToggle()
			}, 3e3)
		}, window.checkWidgetSession = () => {
			let $ = window.localStorage.getItem("AISENSY_WA_WIDGET_SESSION"),
				_ = new Date().getTime();
			return !!$ && !($ < _)
		}, window.createWidgetSession = () => {
			window.localStorage.getItem("AISENSY_WA_WIDGET_SESSION");
			let $ = 0;
			$ = "24_HOUR" === _.openWidgetSessionWindow ? 864e5 : 6048e5;
			let t = new Date(new Date().getTime() + $).getTime();
			window.localStorage.setItem("AISENSY_WA_WIDGET_SESSION", t)
		}, "true" == _.openWidgetByDefault && !v && "dev" != _.env) {
		if ("ALWAYS" === _.openWidgetSessionWindow) openWidgetOnLoad();
		else {
			let z = checkWidgetSession();
			z || openWidgetOnLoad()
		}
	}
}
